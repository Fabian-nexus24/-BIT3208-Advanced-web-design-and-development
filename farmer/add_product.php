...
$farmer = fetch_farmer_by_id(current_user_id());
$errors = [];
$old = [
    'name'        => '',
    'category'    => '',
    'description' => '',
    'location'    => $farmer['farming_location'] ?? $farmer['county'] ?? '',
    'price'       => '',
    'stock_qty'   => '',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify()) {
        ...
        if ($errors === []) {
            $result = create_product(current_user_id(), $validated['data'], $_FILES['product_image'] ?? null);
            if ($result['ok']) {
                flash_set('success', 'Product listed successfully!');
                redirect('farmer/manage_products.php');
            }
            $errors[] = $result['error'] ?? 'Could not add product.';
        }
    }
}

$pageTitle = 'Add Product';
dashboard_shell_start(ROLE_FARMER, 'add-product', $pageTitle);
?>
            ...
            <div class="card-body p-4">
                ...
                <form method="post" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" required value="<?= e($old['name']) ?>" placeholder="e.g. Fresh Tomatoes">
                    </div>
                    <div class="row">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">JPG or PNG, max 2 MB. A default image is used if none uploaded.</div>
                    </div>
